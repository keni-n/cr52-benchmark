...
#define HSFSR_ER                (uint16_t)(1U << 7U)
#define HSFSR_TEND              (uint16_t)(1U << 6U)
#define HSFSR_TDFE              (uint16_t)(1U << 5U)
#define HSFSR_BRK               (uint16_t)(1U << 4U)
#define HSFSR_RDF               (uint16_t)(1U << 1U)
#define HSFSR_DR                (uint16_t)(1U << 0U)

#define HSLSR_ORER              (uint16_t)((uint16_t)1U << 0U)

/* Retrun value */
#define HSCIF_PROC_OK               (0x0U)
#define HSCIF_FIFO_EMPTY            (0x0U)
#define HSCIF_EXIST_DATA_IN_FIFO    (0x1U)

#ifdef V4H
/* for V4H */
#define BASE_HSCIF_ADDR        0xe6540000
#endif
#ifdef X5H
/* for X5H */
#define BASE_HSCIF_ADDR        0xc0710000
#endif
...
uart_send_char(char c)
{
	...
	//while((mem_read16(HSCIF_HSFSR) & HSFSR_TDFE) == 0)
	while(!((HSFSR_TEND | HSFSR_TDFE) & (mem_read16(HSCIF_HSFSR))))
	{
		;    /* Wait TEND=0 and TDFE=0 */
		;    /* Wait TDFE=0 */
	}
	mem_write8(HSCIF_HSFTDR, c);
	reg = mem_read16(HSCIF_HSFSR);
	mem_write16(HSCIF_HSFSR, (reg & ~(HSFSR_TEND | HSFSR_TDFE)));	/* TEND,TDFE clear */
	return;
}
