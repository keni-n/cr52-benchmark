...
/* Local modf: this build links neither libc nor libm. Kept static so it
   never collides with a toolchain-provided modf. */
static double modf(double x, double *iptr) {
    *iptr = (double)((long)x);  /* C cast truncates toward zero */
    return x - *iptr;           /* fractional part */
}
